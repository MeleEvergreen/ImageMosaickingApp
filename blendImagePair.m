...
%BLENDIMAGEPAIR
    ...
    binary_mask = (binary_mask_s + binary_mask_d) > 1;
    ...
    for c = 1:Cs
        ...
        elseif strcmp(mode, 'blend')
            ...
            % ADD YOUR CODE HERE
            % ---------------------------
%             Create masks amplifiers through im2single
            Mask_s = im2single(masks);
             Mask_d = im2single(maskd);

%            calculate the differeneces 
             bitwise = (Mask_s .* Mask_d);
             Masks_s_1 = Mask_s - bitwise;
             Masks_d_1 = Mask_d - bitwise;

%            compute ecludian distance
%             disp(class(Masks_s_1));
%             disp(class(masks));
%             disp(Masks_s_1);

            class_masks = class(masks);
            class_maskd = class(maskd);

            Masks_s_1 = cast(Masks_s_1, class_masks);
            Masks_d_1 = cast(Masks_d_1, class_maskd);

             dist_s = sqrt(sum((Masks_s_1-masks).^2));
             dist_d = sqrt(sum((Masks_d_1-maskd).^2));

             bwdist_s = bwdist(Masks_s_1).*single(masks);
             bwdist_d = bwdist(Masks_d_1).*single(maskd);
             

%             Scale values
            

        Masks_d_2 = ((max(bwdist_d(:)) - bwdist_d) ./ max(bwdist_d(:)) .* binary_mask) + single(maskd);  
        Masks_s_2 = ((max(bwdist_s(:)) - bwdist_s) ./ max(bwdist_s(:)) .* binary_mask) + single(masks);
      
        out_img(:,:,1) = (wrapped_imgs(:,:,1) .* Masks_s_2 + wrapped_imgd(:,:,1) .* Masks_d_2)./(Masks_s_2 + Masks_d_2);
        out_img(:,:,2) = (wrapped_imgs(:,:,2) .* Masks_s_2 + wrapped_imgd(:,:,2) .* Masks_d_2)./(Masks_s_2 + Masks_d_2);
        out_img(:,:,3) = (wrapped_imgs(:,:,3) .* Masks_s_2 + wrapped_imgd(:,:,3) .* Masks_d_2)./(Masks_s_2 + Masks_d_2);
